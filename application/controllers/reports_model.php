...
class Reports_model extends CI_Model{
	...
	function dashboard($organization="",$type="",$state=""){
		...
		$this->db->select("SUM(CASE WHEN visit_type = 'IP' THEN 1 ELSE 0 END) total_ip,
			...
			SUM(CASE WHEN visit_type = 'OP' AND pv.patient_id IN (SELECT p.patient_id FROM patient_visit p WHERE p.admit_date<pv.admit_date AND visit_type='OP' AND hospital_id = hospital.hospital_id) THEN 1 ELSE 0 END) repeat_op",false)
		...
		if($type == "state"){
			...
			$this->db->select('hospital.type6,count(DISTINCT hospital.hospital_id) hospital_count, dashboards.short_name');
			$this->db->join('dashboards','hospital.type6 = dashboards.type6','left');
