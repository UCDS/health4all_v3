...
class Indent_approve extends CI_Controller{                                                 //create Class with name Indent_approve which extends CodeIgniter class controller             
    ...
    {
        $f_indent_date_time = date("d-M-Y H:i:s", strtotime($indent_date_time));
        $f_approve_date_time = date("d-M-Y H:i:s", strtotime($this->input->post('approval_date') . " " . $this->input->post('approval_time')));
    ...
    function indent_approval(){                                                             //definition of a method :indent_approval
        ...
        if($this->form_validation->run() == False){                                         //checking whether validation is true or false
            ...
            $approval_details = $this->indent_approval_model->display_approve_details();
            log_message("info", "SAIRAM: indent date ".$approval_details[0]->indent_date);
            $indent_date_time = $approval_details[0]->indent_date;
