...
<div class="row">
		...
		<input type="hidden" name="page_no" id="page_no" value='<?php echo "$page_no"; ?>'>
			<select class="form-control" id="user_functions" name="user_functions" style="width:50%!important;">
				<option value="">Select User Function</option>
				<?php foreach($user_functions as $uf) { ?>
					<option <?php if($this->input->post('user_functions')==$uf->user_function_id){ echo "selected"; } ?> 
					value="<?php echo $uf->user_function_id; ?>"><?php echo $uf->user_function.' - '.$uf->description; ?></option>
				<?php } ?>
			</select>
...
<?php if(isset($related_users) && count($related_users)>0)
{ ?>
...
<?php 
	...
	$total_records = $related_users_count[0]->count ;
...
echo "</select></li>";
	...
	Please select atleast one user function to display data.
<?php }  ?>
</div>	
