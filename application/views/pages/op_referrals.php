...
<h5>Page <?php echo $page_no." of ".$total_no_of_pages." (Total ".$total_records.")" ; ?></h5>
	...
	<tr>
		...
		<td><?php echo $s->visit_name. ", ". $s->op_department;?></td>		
		<td><?php 
		if(isset($s->latest_op_no) && $s->latest_op_no!="") {
		echo $s->latest_op_no;?> , <?php echo date("j M Y", strtotime("$s->admit_date")).", ".date("h:i A.", strtotime("$s->admit_time"));
		}?></td>		
		<td><?php echo $s->op_icd_code." - ".$s->code_title;?></td>
