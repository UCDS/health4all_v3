<link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/suggestive_text/jquery-ui.css">
...
<script>
	...
	var SuggestiveText = $.noConflict();
</script>

<script src="<?php echo base_url();?>assets/js/jquery-ui.min.js"></script>
<script>
		SuggestiveText(function(){
			
		 <?php foreach($order as $test){
		  $tags =  "var text_result_".$test->test_id."_tag = [ ";
		  foreach($all_suggestions as $suggestion){
		  	if($test->test_master_id==$suggestion->test_master_id)
		  		$tags.="'".$suggestion->suggestion."',";
		  }
		  echo $tags."];";
		  echo "SuggestiveText( '#text_result_"."$test->test_id"."').autocomplete({
      			source: text_result_".$test->test_id."_tag
    		  });";
		  } ?>
	});
	</script>
...
<?php if(isset($order)){ 
	...
	<?php 
		...
		echo "<datalist id='SuggestiveList".$test->test_id."'>";
		 foreach($all_suggestions as $suggestion){
		 	if($test->test_master_id==$suggestion->test_master_id)
				echo "<option value= '$suggestion->suggestion' />";		
		}
			echo '</datalist>';
		}	
	?>	
