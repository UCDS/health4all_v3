...
class Indent_issue extends CI_Controller{                                                         //create Class with name Indent issue which extends CodeIgniter class controller 
 ...
 function indent_issued(){                                                                        //definition of a method :indent_approval
        ...
        if($this->form_validation->run() == False){                                               //checking whether validation is true or false
            ...
            $issue_details = $this->indent_issue_model->display_issue_details();
            ...
            foreach($issue_details as $issued){
            ...
            if(!$this->validate_date_time($issue_details[0]->approve_date_time) || $this->form_validation->run() == False){
                $this->data['mode']="select";                                                         //save select in data array of index:mode
                $this->data['indent_issued']=$issue_details;      //get data from display_issue_details method of indent_issue_model and store it into data array of index:indent_issued
